...
export const PixelEditor: React.FC = () => {
  ...
  const {
    buffer, version, canUndo, canRedo,
    getBuffer, getPixelColor,
    beginStroke, paintPixel, endStroke,
    commit, floodFill, undo, redo, reset,
  } = usePixelCanvas(width, height);

  const handleCanvasCreate = useCallback((w: number, h: number) => {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (!meta) {
        ...
        if (key === '+' || key === '=') { handleZoom(1); return; }
        if (key === '-') { handleZoom(-1); return; }
      }
    };
    ...
    <div className="h-screen flex flex-col bg-neutral-950 text-white overflow-hidden">
      ...
      <header className="flex items-center justify-between px-4 h-12 border-b border-neutral-800 shrink-0">
        <div className="flex items-center gap-3">
          <span className="text-indigo-400 font-bold text-lg tracking-tight">lowpix</span>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={() => setBackground(b => b === 'transparent' ? 'white' : 'transparent')}
          ...
          <div className="flex items-center gap-1">
            ...
            >−</button>
            <span className="text-neutral-400 text-xs w-10 text-center">{zoom}px</span>
            <button
              onClick={() => handleZoom(1)}
              className="w-6 h-6 text-neutral-400 hover:text-white hover:bg-neutral-800 rounded transition-colors text-sm flex items-center justify-center"
            >+</button>
